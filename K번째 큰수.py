...
#내가 푼 코드
n, k = map(int, input().split())
a = (list(map(int, input().split()))) # [13, 15, 34, 23, 45, 65, 33, 11, 26, 42]

a = sorted({a[i]+ a[j]+a[z] for i in range(n) for j in range(i+1, n) for z in range(j+1, n)}, reverse=True)
#중복 제거하고자 set Comprehension 사용 sorted 쓰면 자동으로 list 됨.  reverse=True 로 내림차순 정렬
print(a[k-1])